...
for enum, file in enumerate(file_list):
    ...
    alg_path = "./figures/" + alg_list[enum] + "-pyth_bypass/"
...
plt.savefig("figures/bypass_steer.pdf", bbox_inches='tight', dpi=300)
plt.close()
...
for enum, file in enumerate(file_list):
    order = len(file_list) - enum
    alg_path = "./figures/" + alg_list[enum] + "-pyth_encounter/"
...
plt.xlim(-3, 3)
# plt.grid()
plt.savefig("figures/encounter_acc.pdf", bbox_inches='tight', dpi=300)
plt.close()
