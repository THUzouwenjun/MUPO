...
def get_apprfunc_dict(key: str, Atype=None, **kwargs):
    var = dict()
    ...
    var["Lipschitz_constant"] = kwargs.get(key + "_Lipschitz_constant", None)
    ...
    if kwargs["action_type"] == "continu":
        ...
        if ("action_shape" in kwargs) and (kwargs["action_shape"] is not None):
            var["act_shape"] = kwargs["action_shape"]
        else:
            var["act_shape"] = None
...
class ModuleOnDevice:
    def __init__(self, module, device):
        self.module = module
        self.prev_device = next(module.parameters()).device
    ...
    def __enter__(self):
        if self.different_device:
            ...
            # print("time for change device [ms]: ", (time.time() - start_time) * 1000)
