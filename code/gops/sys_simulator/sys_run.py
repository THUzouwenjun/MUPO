...
from gops.utils.plot_q import plot_q_function_3D, plt_action_disconti, VideoCreator
...
class PolicyRunner:
    ...
    def __init__(
        ...
        # save path
        path = os.path.join(os.path.dirname(__file__), "..", "..", "../results/figures")
    ...
    def run_an_episode(
        ...
        while not (done or info["TimeLimit.truncated"]):
            ...
            if is_opt:
                ...
                with torch.no_grad():
                    ...
                    save_path = self.save_path + "/q/" + str(step) + ".png"
                    os.makedirs(self.save_path + "/q", exist_ok=True)
                    # plot_q_function_3D(obs, controller, save_path)
                    save_path_pi = self.save_path + "/pi/" + str(step) + ".png"
                    os.makedirs(self.save_path + "/pi", exist_ok=True)
                    # plot_pi_2D(obs, controller, save_path_pi)
            if self.use_dist:
                action = np.hstack((action, env.dist_func(step * env.tau)))
            next_obs, reward, done, info = env.step(action)
            # if action is a scalar, convert it to list
            if not (hasattr(action, "shape") and len(action.shape) > 0):
            ...
            if self.constrained_env:
                print("constraint", np.clip(info["constraint"], a_min=-3.0, a_max=20.0))
                constrain_list.append(np.clip(info["constraint"], a_min=-3.0, a_max=10.0))
            if self.is_tracking:
        ...
        # # 将self.save_path + "/q/"下的图片合成mp4
        if not is_opt:
            # 调用ffmpeg将图片合成视频，图片命名规则为0.png,1.png,2.png...
            # 使用H.264编码，高质量
            creator = VideoCreator(self.save_path)
            creator.create_video()
            os.system(
                "ffmpeg -r 30 -i {}/q/%d.png -vcodec libx264 -pix_fmt yuv420p {}/videos/q.mp4".format(
                    self.save_path, self.save_path
                )
            )
            # os.system(
            #     "ffmpeg -r 30 -i {}/pi/%d.png -vcodec libx264 -pix_fmt yuv420p {}/videos/pi.mp4".format(
            #         self.save_path, self.save_path
            #     )
            # )
        eval_dict = {
    ...
    def compute_action(self, obs: np.ndarray, networks: Any) -> np.ndarray:
        ...
        if self.continuous:
            action = networks.compute_action(batch_obs, deterministic=True, continuous=True)
        else:
            action = networks.compute_action(batch_obs, deterministic=True)
    ...
    def draw(self):
        ...
        for j in range(action_dim):
            ...
            for i in range(policy_num):
                ...
                # 画非连续曲线,color 31 119 180
                color_list = [(31/255, 119/255, 180/255), (255/255, 127/255, 14/255), (44/255, 160/255, 44/255), (214/255, 39/255, 40/255), (148/255, 103/255, 189/255), (140/255, 86/255, 75/255), (227/255, 119/255, 194/255), (127/255, 127/255, 127/255), (188/255, 189/255, 34/255), (23/255, 190/255, 207/255)]
                plt_action_disconti(step_array[i], action_array[i, :, j], label="{}".format(legend), color=color_list[i])
    ...
    def __run_data(self):
        for i in range(self.policy_num):
            ...
            if hasattr(env, "set_is_closed_loop"):
                env.set_is_closed_loop(self.init_info["is_closed_loop"])
            ...
            print("trained_policy_iteration", trained_policy_iteration)
        ...
        if self.use_opt:
            ...
            if self.opt_args["opt_controller_type"] == "OPT":
                ...
                raise NotImplementedError("MPC is not supported yet.")
