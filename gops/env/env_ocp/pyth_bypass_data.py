...
#  Update: 2024-04-26, Wenjun Zou: modify environment
...
from gops.env.env_ocp.pyth_veh3dofconti_data import SimuVeh3dofconti, angle_normalize, ego_vehicle_coordinate_transform
...
class SimuVeh3dofcontiSurrCstr(SimuVeh3dofconti):
    ...
    def reset(
        ...
        **kwargs,
        ...
        # get surrounding vehicle distance from kwargs
        surr_veh_dist = kwargs.get("surr_veh_dist", self.np_random.uniform(10.0, 25.0))
        # print("surr_veh_dist: ", surr_veh_dist)
    ...
    def get_obs(self) -> np.ndarray:
        ...
        surr_obs_rel = np.concatenate(
            ([surr_x_tf[0], surr_y_tf[0], surr_phi_tf[0],], self.surr_state[:, 3]))
        return np.concatenate((obs, surr_obs_rel))

    def set_is_closed_loop(self, is_closed_loop):
        self.is_closed_loop = is_closed_loop

    def step_open_loop(self,):
        self.state[1] = self.state[1] + 0.02 * 2/3
    ...
    def compute_reward(self, action: np.ndarray) -> float:
        ...
        threshold = 0.0 if self.is_eval else -0.1
